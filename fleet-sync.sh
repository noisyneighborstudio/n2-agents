...
sync_addr_class()   { printf '%s' "$1" | cut -d'|' -f1; }
sync_addr_profile() { printf '%s' "$1" | cut -d'|' -f2; }
sync_addr_vendor()  { printf '%s' "$1" | cut -d'|' -f3; }
sync_addr_relpath() { printf '%s' "$1" | cut -d'|' -f4-; }
...
sync_auth_reason() {
  case $1 in
    ...
    claude) echo "interactive login lives in the macOS login keychain, which profile isolation does not isolate; only env/API-key material is file-visible" ;;
...
sync_manifest() {
  ...
  all_profiles 2>/dev/null | while IFS= read -r p; do
    ...
    sync_vendor_list | while IFS= read -r v; do
      ...
      find -L "$slot" -type f 2>/dev/null | while IFS= read -r f; do
        rel=${f#"$slot"/}
        [ "$rel" = "$f" ] && continue
...
sync_write() {  # sync_write <addr> <srcfile|''=delete> [expected-digest] [peer] [force]
  ...
  if [ "$sw_c" != auth ] && sync_file_carries_secret "$2"; then
...
sync_absorb_locked() {
  ...
  if [ -n "$sa_sb" ] && [ "$sa_sb" = "$sa_l" ] && [ "$sa_l" != "$3" ]; then
    case $sa_w in
      push|conflict) sa_w=pull; sync_conflict_drop "$1" ;;
    esac
  fi
...
# Returns: ok | install | update | defer | unmanaged
sync_tool_state() {  # sync_tool_state <name>
  stl=$(sync_tool_line "$1") || { echo unmanaged; return 0; }
...
agents fleet tools <verb>
  ...
  list                                 the fleet-managed manifest
...
cmd_fleet_tools() {
  ...
  case $tv in
    ...
    list|add|rm|status|apply|install|deferred) ;;
  ...
  case $tv in
    list)
      while IFS= read -r line; do
        case $line in ''|'#'*) continue ;; esac
        if sync_tool_line_ok "$line"; then
          printf '%s|%s|%s|%s|%s\n' "$(sync_tool_field "$line" 1)" \
                 "$(sync_tool_value "$line" 2)" "$(sync_tool_value "$line" 3)" \
                 "$(sync_tool_value "$line" 4)" "$(sync_tool_field "$line" 5)"
        else
          printf 'invalid|%s\n' "$(printf '%s' "$line" | cut -d'|' -f1)"
        fi
    ...
    install)
      ...
      l=$(sync_tool_line "$1"); st=$(sync_tool_state "$1")
      [ "$st" = ok ] && { printf 'ok\t%s\n' "$1"; return 0; }
      ...
      if sh -c "$(sync_tool_value "$l" 4)" >/dev/null 2>&1 && [ "$(sync_tool_state "$1")" = ok ]; then
