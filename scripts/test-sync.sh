...
check  "scope: mcp config is in scope"       "mcp|Work|claude|.mcp.json" "$sc"
...
cid=$(peer alpha fleet sync conflicts 2>/dev/null | awk -F'\t' '$2=="tools|-|-|manifest"{print $1}' | tail -1)
[ -n "$cid" ] || cid=$(peer alpha fleet sync conflicts 2>/dev/null | grep 'tools|-|-|manifest' | awk '{print $1}' | tail -1)
if [ -n "$cid" ]; then ok "tools: the divergence is a visible conflict"; else bad "tools: the divergence is a visible conflict" "no conflict id for the manifest"; fi
peer alpha fleet sync resolve "$cid" --local >/dev/null 2>&1
out=$(peer alpha fleet sync now --peer "$B" 2>&1)
...
same  "tools: the controlled installer ran on arrival, with no second command" "2.0" "$(cat "$base/beta/demotool.v" 2>/dev/null)"
...
out=$(peer pi fleet sync auth enable gemini 2>&1); rc=$?
...
same  "toolexc: withdrawn and resolved, the controlled installer runs" "3.0" "$(cat "$base/tex2/exctool.v" 2>/dev/null)"
