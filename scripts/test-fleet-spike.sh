...
cp "$repo/agents" "$repo/workspace-pack.py" "$repo/usage.py" "$repo/codex-rpc.py" "$repo/codex-run.py" "$repo/usage-store.py" "$repo/vendors.sh" "$repo/fleet.sh" "$repo/fleet-sync.sh" "$repo/fleet-exec.sh" "$repo/fleet-qa-import.py" "$repo/profile-metadata.py" "$base/bundle/"
