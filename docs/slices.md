...
1. **Coordinated migration recovery** (`migration-abort-peers`). Behavior: explicitly abandon one exact prepared migration across its participants before restoring the coordinator's legacy route. Proof: disposable two-peer CLI recovery, lost acknowledgement and retry, unreachable participant leaves coordinator pending, late prepare cannot resurrect an abandoned migration, stale revision cannot clear a new marker, and all credential bytes remain unchanged. Scope: signed recovery and durable tombstones only; no credential deletion, grant activation, or migration-complete claim. Planned diff: 250 lines; stop at 500.

2. **Retirement evidence spike** (`migration-retirement-spike`). Knowledge: define the next observable step for retiring legacy copies and activating an owner grant from the ownership contract and provider documentation. Proof: inspect disposable file/conflict/keychain inventory fixtures and record which evidence permits advancement and which remains unknown. Scope: notes and fixtures only; no real credentials, revocation, or fresh login. Replace this entry with one bounded implementation slice.
